...
{#if isOpen}
  ...
  >
    ...
    <div
      class="relative flex w-full max-w-2xl flex-col overflow-hidden border border-zinc-800 bg-zinc-950/90 shadow-2xl shadow-black/50 outline-none"
    ...
    >
      ...
      <header class="flex items-center justify-between border-b border-zinc-800/50 bg-zinc-900/30 px-6 py-4">
        ...
        <button 
          type="button"
          class="flex h-8 w-8 cursor-pointer items-center justify-center text-zinc-500 transition-all hover:bg-white/5 hover:text-zinc-100" 
          onclick={onClose}
        >
          <X size={18} />
        </button>
      ...
      <div class="custom-scrollbar max-h-[450px] overflow-y-auto bg-zinc-950/20 p-2">
        {#if $recentProjects.length === 0}
          <div class="flex flex-col items-center justify-center py-20 text-center">
            <div class="mb-4 flex h-16 w-16 items-center justify-center bg-zinc-900 text-zinc-700">
              <FolderOpen size={32} />
            </div>
        ...
        {:else}
          <div class="flex flex-col gap-1">
            {#each $recentProjects as project, index}
              ...
              <!-- svelte-ignore a11y_no_static_element_interactions -->
              <div
                class="project-item group relative flex w-full cursor-pointer items-center gap-4 border border-transparent px-4 py-3 text-left transition-all duration-150 hover:bg-white/[0.03]"
                onclick={() => openSelectedProject(index, project)}
              >
                <div class="flex h-10 w-10 shrink-0 items-center justify-center bg-zinc-900 text-zinc-500 transition-colors group-hover:text-emerald-400">
                  <FolderOpen size={20} />
                </div>
                ...
                {#if git}
                  <div class="flex items-center gap-4 shrink-0 px-4">
                    {#if git.is_repo}
                      <div class="flex items-center gap-1.5 border border-zinc-800 bg-zinc-900/50 px-2.5 py-1">
                      ...
                      {:else}
                        <span class="flex items-center gap-1.5 border border-emerald-500/20 bg-emerald-500/5 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-emerald-500/70">
                ...
                <button 
                  type="button"
                  class="flex h-8 w-8 shrink-0 cursor-pointer items-center justify-center text-zinc-700 transition-all hover:bg-red-500/10 hover:text-red-400 opacity-0 group-hover:opacity-100"
      ...
      <!-- <footer class="border-t border-zinc-800/50 bg-zinc-900/30 px-6 py-3">
        <div class="flex items-center justify-end text-[11px] text-zinc-500 font-medium">
          <div class="flex items-center gap-1.5">
            <kbd class="flex min-w-[20px] items-center justify-center border border-zinc-700 bg-zinc-800 px-1 py-0.5 font-mono text-[10px] text-zinc-300 shadow-sm">Esc</kbd>
