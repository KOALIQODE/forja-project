<script lang="ts">
  import { Info, Loader2, Search, ShieldAlert, ShieldCheck } from '@lucide/svelte';
  ...
  async function handleValidate() {
    ...
    try {
      results = await validateDependency(ecosystem, packageName, version || undefined);
    } catch (value: unknown) {
...
<div class="flex h-full flex-col border border-zinc-800/50 bg-[#0a0a0a] p-4 font-sans text-zinc-300" data-program-ui>
  <div class="mb-4 flex items-center gap-2 border-b border-zinc-800 pb-2">
    <ShieldCheck size={18} class="text-emerald-500" />
    <h2 class="text-sm font-semibold uppercase tracking-wider">Dependency Validator</h2>
  </div>

  {#if availableEcosystems.length === 0}
    <div class="flex flex-1 flex-col items-center justify-center gap-3 text-center text-zinc-500">
    ...
    <div class="mb-6 flex flex-col gap-3">
      <div class="flex gap-2">
        {#each availableEcosystems as option}
          <button
            class="flex flex-1 items-center justify-center gap-2 border px-3 py-1.5 text-[11px] font-mono {ecosystem === option.id ? 'border-zinc-600 bg-zinc-800 text-white' : 'border-zinc-800 text-zinc-500 hover:bg-zinc-900'}"
      ...
      <div class="flex flex-col gap-1">
        <label for="pkg-name" class="text-[10px] uppercase tracking-widest text-zinc-500">Package Name</label>
        <input
          id="pkg-name"
          type="text"
          bind:value={packageName}
          placeholder={ECOSYSTEM_OPTIONS[ecosystem]?.packagePlaceholder ?? 'Package name'}
      ...
      <div class="flex flex-col gap-1">
        <label for="pkg-ver" class="text-[10px] uppercase tracking-widest text-zinc-500">Version (Optional)</label>
        <input
          id="pkg-ver"
          type="text"
          bind:value={version}
          placeholder={ECOSYSTEM_OPTIONS[ecosystem]?.versionPlaceholder ?? 'Version'}
          class="w-full border border-zinc-800 bg-zinc-900/50 px-3 py-2 text-xs font-mono focus:border-zinc-600 focus:outline-none"
          onkeydown={(event) => event.key === 'Enter' && handleValidate()}
        />
      </div>
  ...
  <div class="min-h-0 flex-1 overflow-y-auto">
    {#if error}
      <div class="flex items-start gap-2 border border-red-900/50 bg-red-950/30 p-3">
        <ShieldAlert size={16} class="mt-0.5 shrink-0 text-red-500" />
        <p class="text-xs text-red-400">{error}</p>
      </div>
    {:else if results}
      {#if results.length === 0}
        <div class="flex flex-col items-center justify-center py-12 text-zinc-600 opacity-60">
      ...
      {:else}
        <div class="space-y-3">
          <div class="mb-2 flex items-center gap-2 text-red-400">
            <ShieldAlert size={16} />
            <span class="text-xs font-semibold">{results.length} Vulnerabilities Found</span>
          </div>
          {#each results as vuln}
            <div class="space-y-2 border border-zinc-800/50 bg-zinc-900/40 p-3">
              <div class="flex items-center justify-between">
                <span class="text-xs font-mono font-bold text-white">{packageName}</span>
                <span class="bg-zinc-800 px-1.5 py-0.5 text-[10px] uppercase {severityColor[vuln.severity]}">
                  {vuln.severity}
                </span>
              </div>
              ...
              <div class="flex flex-wrap gap-2 text-[10px]">
                {#if vuln.id}
                  <span class="text-zinc-500">ID: <span class="font-mono text-zinc-300">{vuln.id}</span></span>
                {/if}
                {#if vuln.patched_versions}
                  <span class="text-zinc-500">
                    Fixed in: <span class="font-mono text-emerald-500/80">{vuln.patched_versions}</span>
                  </span>
                {/if}
              </div>
              {#if vuln.url}
                <a href={vuln.url} target="_blank" rel="noopener" class="block text-[10px] text-emerald-500 hover:underline">
                  View Advisory →
                </a>
              {/if}
            </div>
  ...
  <div class="mt-4 border-t border-zinc-800 pt-4">
    <div class="flex items-start gap-2 text-[10px] italic text-zinc-500">
      ...
      <p>This validation creates a temporary sandbox to audit the package without modifying your project's manifests.</p>
    </div>
  </div>
</div>
