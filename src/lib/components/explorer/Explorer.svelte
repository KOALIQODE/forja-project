<script lang="ts">
  ...
  // Monitor git status changes reactively
  $effect(() => {
    if (effectiveRoot) {
      void startGitStatusMonitoring(effectiveRoot);
    } else {
      void stopGitStatusMonitoring();
    }
  });

  // Context Menu State
  let contextMenu = $state<{ x: number; y: number; options: any[] } | null>(
    null,
  );
    ...
    if (entry.is_dir) {
      if (viewMode === "drill") {
        ...
        if ($expandedPaths.has(entry.path) && !directoryCache.get(entry.path)) {
          try {
            const res = await invoke<FileEntry[]>("explore_directory", {
              path: entry.path,
            });
  ...
  async function deleteEntry(entry: FileEntry) {
    const confirmed = await ask(
      `¿Estás seguro de que deseas eliminar "${entry.name}"?`,
      {
    ...
    if (entry) {
      if (entry.is_dir) {
        options.push({
          label: "New File",
          icon: Plus,
          onClick: () => createNewFile(entry.path),
        });
        options.push({
          label: "New Folder",
          icon: FolderPlus,
          onClick: () => createNewDirectory(entry.path),
        });
        options.push({ separator: true });
        options.push({
          label: "Pin as Root",
          icon: Pin,
          onClick: () => pinFolder(entry.path),
        });
      }
      options.push({
        label: "Rename",
...
>
  <header class="shrink-0 bg-(--forja-ui-explorer-bg,#0a0a0a) p-3 pb-2">
      ...
      {#if hasProject}
        <div class="flex items-center gap-0.5">
          ...
          <button
            ...
            title={canOpenDependencyValidator
    ...
    {#if hasProject}
      <div class="flex min-h-[26px] items-center gap-2 px-2 py-0.5">
  ...
  {#if showValidator}
    <div class="flex-1 min-h-0 border-t border-zinc-800 flex flex-col">
      <div
        class="flex items-center justify-between px-3 py-1 bg-zinc-900/50 shrink-0"
      >
        <span class="text-[9px] uppercase tracking-widest text-zinc-500"
          >Security Sandbox</span
        >
        <button
