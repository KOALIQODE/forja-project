<script lang="ts">
  ...
  import type { PluginPreflightInfo } from "$lib/utils/shared/pluginClient";
...
>
  ...
  <!-- Panel -->
  <div class="relative flex w-full max-w-md flex-col overflow-hidden bg-(--color-surface-base) border border-(--color-border) shadow-2xl">
    ...
    <div class="flex items-center justify-between border-b border-(--color-border) px-6 py-5">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="text-[9px] font-bold uppercase tracking-widest text-(--color-text-muted)">Permission Request</p>
          <h2 class="text-base font-semibold text-(--color-text-primary)">{info.name}</h2>
        </div>
      </div>
      <button
        onclick={onDeny}
        class="p-1.5 text-(--color-text-muted) transition-colors hover:bg-(--color-surface-hover) hover:text-(--color-text-primary)"
        aria-label="Deny"
      >
        <X size={16} />
      </button>
    ...
    <!-- Plugin meta -->
    <div class="flex items-center gap-3 border-b border-(--color-border) bg-(--color-surface-hover)/30 px-6 py-3">
      <div class="flex h-8 w-8 items-center justify-center bg-black/40 text-(--color-text-muted)">
        {#if info.kind === "theme"}
          <Palette size={16} />
        {:else}
          <Puzzle size={16} />
      ...
      <div class="text-xs text-(--color-text-secondary)">
        ...
        <span class="mx-1.5 text-(--color-text-muted)">·</span>
    ...
    <div class="px-6 py-5">
      ...
      {:else}
        <p class="mb-3 text-[10px] uppercase tracking-widest text-(--color-text-muted)">
        ...
        <ul class="flex flex-col gap-2">
          {#each info.permissions as perm}
            ...
            <li class="perm-item group relative flex w-full items-start gap-3 px-3 py-2.5 {riskClass(perm.risk)} border border-transparent hover:border-current/10">
    ...
    <!-- Action buttons -->
    <div class="flex gap-3 border-t border-(--color-border) bg-(--color-surface-hover)/30 px-6 py-4">
      <button
        onclick={onDeny}
        class="flex flex-1 items-center justify-center gap-2 border border-(--color-border) bg-(--color-surface-base) py-2.5 text-xs font-medium text-(--color-text-secondary) transition-all hover:bg-(--color-surface-hover) hover:text-(--color-text-primary)"
      >
        <X size={13} />
        Deny
      </button>
      <button
        ...
        class="flex flex-1 items-center justify-center gap-2 py-2.5 text-xs font-semibold transition-all
          {overallRisk === 'high'
            ...
            : 'bg-(--color-accent) text-(--color-surface-base) hover:opacity-90'}"
      >
        <ShieldCheck size={13} />
...
<style>
  .perm-item { border-radius: 4px; transition: all 0.12s; }
  .perm-item + .perm-item { margin-top: 2px; }
</style>
