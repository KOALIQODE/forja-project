<script lang="ts">
  ...
  import { UI_TEXT } from "$lib/utils/constants.js";
  ...
  function deleteProject(index: number) {
    recentProjects.update((projects) => {
      ...
      if (selectedIndex >= updated.length)
        selectedIndex = Math.max(0, updated.length - 1);
      return updated;
    });
  }
  ...
  function handleKeydown(e: KeyboardEvent) {
    if (e.key === "Escape") {
      ...
      e.preventDefault();
      selectedIndex =
        (selectedIndex - 1 + $recentProjects.length) % $recentProjects.length;
    } else if (e.key === "Enter") {
      e.preventDefault();
  ...
  >
    <div class="recent-shell flex w-full max-w-2xl flex-col overflow-hidden">
      ...
      <div class="header-row flex items-center gap-3 px-4 py-2.5">
        ...
        <button
          onclick={onClose}
          class="close-btn flex h-6 w-6 items-center justify-center"
        >
          <X size={14} />
        </button>
      </div>
      ...
      <div class="custom-scrollbar max-h-[400px] overflow-y-auto">
        {#if $recentProjects.length === 0}
          <div
            class="empty-state flex flex-col items-center justify-center gap-2 py-16"
          >
            <FolderOpen size={24} strokeWidth={1} />
            <p class="text-[10px] uppercase tracking-widest">
              {UI_TEXT.NO_RECENT_PROJECTS}
            </p>
          </div>
        {:else}
          {#each $recentProjects as project, index}
            ...
            >
              ...
              <div class="flex min-w-0 flex-1 flex-col">
                <span
                  class="item-name truncate text-[12px]"
                  class:item-name--active={selectedIndex === index}
                >
                  ...
                  {#if git.is_repo}
                    ...
                    {#if git.has_upstream}
                      <div class="flex items-center gap-1.5">
                        <span
                          class="flex items-center gap-0.5 font-mono text-[10px]"
                          class:git-ahead={git.ahead > 0}
                        >
                          <ArrowUp size={10} />{git.ahead}
                        </span>
                        <span
                          class="flex items-center gap-0.5 font-mono text-[10px]"
                          class:git-behind={git.behind > 0}
                        >
                    ...
                    {:else}
                      <span
                        class="git-publish flex items-center gap-1 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider"
                      >
                        <Globe size={9} />{UI_TEXT.PUBLISH}
                      </span>
                    {/if}
                  {:else}
                    <span class="git-none text-[9px] uppercase tracking-widest"
                      >{UI_TEXT.NO_GIT}</span
                    >
                  {/if}
                </div>
      ...
      <!-- Footer -->
      <div
        class="footer-row flex items-center justify-between px-3 py-1.5 text-[9px] uppercase tracking-[0.12em]"
      >
