<script lang="ts">
  ...
  let activeTab = $state<'overview' | 'vulns' | 'outdated' | 'install'>('overview');
  let search = $state('');
  ...
  let filtered = $derived(
    allDeps.filter((dependency) => {
      ...
      const matchesSearch = !query || dependency.name.toLowerCase().includes(query);
      const matchesEcosystem = !filterEcosystem || dependency.ecosystem === filterEcosystem;
  ...
  let outdatedDeps = $derived(allDeps.filter((dependency) => dependency.is_outdated));
  ...
  function maxSeverity(dep: Dependency) {
    ...
    return Math.max(...dep.vulnerabilities.map((vulnerability) => scores[vulnerability.severity] ?? 0), 0);
  }
  ...
  async function handleInstall() {
    ...
    try {
      installOutput = await installDep(manifestPath, pkgName.trim(), pkgVersion || null, pkgDev);
  ...
  let tabs = $derived([
    ...
    { id: 'vulns', label: 'vulnerabilities', count: vulnDeps.length, danger: vulnDeps.length > 0 },
    { id: 'outdated', label: 'outdated', count: outdatedDeps.length, danger: false },
...
<div class="deps-panel" data-program-ui>
  <header class="panel-header">
    ...
    <div class="header-right">
      <select bind:value={filterEcosystem} class="eco-filter" disabled={$availableEcosystems.length === 0}>
        <option value="">all ecosystems</option>
        {#each $availableEcosystems as ecosystem}
          <option value={ecosystem}>{ecosystem}</option>
        {/each}
      </select>
      <button class="btn" onclick={() => scanAll(projectRoot)} disabled={!projectRoot || $isScanning}>
        {#if $isScanning}
  ...
  {:else if !$hasDetectedManifests}
    <div class="empty-panel">
      <p class="empty-title">No supported dependency manifests were detected.</p>
      <p class="empty-copy">Nothing was sent to npm, cargo, or any other audit tool for this project.</p>
    </div>
  {:else}
    <div class="summary-grid">
      ...
      <div class="stat-card danger">
        ...
        <div class="stat-val">{$globalSummary.critical + $globalSummary.high}</div>
      </div>
      <div class="stat-card warn">
        <div class="stat-label">moderate / low</div>
        <div class="stat-val">{$globalSummary.moderate + $globalSummary.low}</div>
      </div>
    ...
    {#if !$hasDependencies}
      <div class="empty-banner">
        <p>Manifests were detected, but they do not declare dependencies yet. Audit and outdated checks are skipped until packages exist.</p>
      </div>
    {/if}
    ...
    {#if activeTab === 'overview'}
      <div class="search-row">
        <input type="text" bind:value={search} placeholder="search packages..." />
      </div>
      <div class="dep-list">
        {#each filtered as dep (`${dep.manifest}:${dep.name}`)}
          ...
          <div
            ...
            onclick={() => expandedDep = expandedDep === `${dep.manifest}:${dep.name}` ? null : `${dep.manifest}:${dep.name}`}
          >
            <div class="dep-main">
              ...
              {#each dep.vulnerabilities as vulnerability}
                <span
                  class="sev-badge"
                  style="background-color: {severityColor(vulnerability.severity)}22; color: {severityColor(vulnerability.severity)}"
                >
                ...
                {#each dep.vulnerabilities as vulnerability}
                  <div class="vuln-row">
                    ...
                    {#if vulnerability.patched_versions}
                      <span class="fix-hint">fix: {vulnerability.patched_versions}</span>
                    {/if}
                    {#if vulnerability.url}
                      <a href={vulnerability.url} target="_blank" rel="noopener" class="vuln-link">↗</a>
                    {/if}
                  </div>
                {/each}
              </div>
        ...
        {#if filtered.length === 0}
          <div class="empty-state">
            {$hasDependencies ? 'No packages matched the current filters.' : 'No dependencies declared yet.'}
          </div>
        {/if}
      </div>
            ...
            <div class="vuln-list">
              {#each dep.vulnerabilities as vulnerability}
                <div class="vuln-row">
                  ...
                  <span class="vuln-title">{vulnerability.title}</span>
                  {#if vulnerability.patched_versions}
                    <span class="fix-hint">patched in {vulnerability.patched_versions}</span>
                  {/if}
                </div>
              {/each}
            </div>
    ...
    {:else}
      <div class="install-box">
        ...
        <div class="install-form">
          ...
          <input type="text" bind:value={pkgVersion} placeholder="version (optional)" />
          <label class="dev-toggle">
            <input type="checkbox" bind:checked={pkgDev} /> dev dependency
          </label>
          <button class="btn primary" onclick={handleInstall} disabled={!canInstallIntoManifest || installing || !pkgName.trim()}>
...
<style>
  ...
  .watcher-dot { width: 7px; height: 7px; border-radius: 50%; background: #888; }
  .watcher-dot.active { background: #639922; animation: pulse 2s infinite; }
  ...
  .stat-card { border-radius: 8px; background: rgba(255,255,255,0.03); padding: 10px 12px; }
  .stat-label {
    margin-bottom: 4px;
    color: #888;
    font-size: 11px;
  ...
  .stat-val { color: #f4f4f5; font-size: 22px; font-weight: 500; }
  .stat-card.danger .stat-val { color: #E24B4A; }
  .stat-card.warn .stat-val { color: #BA7517; }
  .stat-card.info .stat-val { color: #185FA5; }

  .manifest-row { display: flex; flex-wrap: wrap; gap: 6px; }
  .manifest-pill {
    display: flex;
    align-items: center;
    gap: 5px;
  ...
  .manifest-pill:hover { background: rgba(255,255,255,0.05); }
  .manifest-meta { color: #71717a; font-size: 10px; }
  .vuln-badge {
    border-radius: 99px;
    background: #FCEBEB;
    padding: 1px 5px;
  ...
  .empty-title { color: #d4d4d8; font-size: 13px; font-weight: 500; }
  .empty-copy { max-width: 34ch; line-height: 1.5; font-size: 12px; }
  ...
  .tab.active { border-bottom-color: #639922; color: #f4f4f5; }
  .tab-count {
    border-radius: 99px;
  ...
  .tab-count.danger { background: rgba(226,75,74,0.12); color: #E24B4A; }
  ...
  .dep-item.has-vuln { border-color: rgba(226,75,74,0.2); background: rgba(226,75,74,0.05); }
  .dep-item.outdated { border-color: rgba(24,95,165,0.2); background: rgba(24,95,165,0.05); }
  .dep-main {
    display: flex;
    flex-wrap: wrap;
    align-items: center;
  ...
  .dep-name { color: #f4f4f5; font-weight: 600; }
  .dep-ver, .dep-latest, .dep-type, .dep-eco, .sev-badge, .fix-hint {
    font-size: 11px;
  }
  .dep-latest { color: #9cc3ea; }
  .dep-type, .dep-eco {
    border-radius: 99px;
  ...
  .vuln-title { color: #d4d4d8; }
  .vuln-link { color: #9cc3ea; text-decoration: none; }
  ...
  .install-target strong { color: #f4f4f5; text-transform: uppercase; }
  .install-target code { color: #9cc3ea; }
  .install-form {
    display: grid;
    gap: 8px;
  }
