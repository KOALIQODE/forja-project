//! Document domain model — rope-backed buffer with incremental tree-sitter parsing.
//!
//! This module is the heart of the editor's document model. It owns:
//! - [`DocumentState`]: a single open file (rope buffer + live AST + highlight query).
//! - [`DocumentManager`]: registry of all open documents, keyed by [`DocumentId`].
//!
//! No I/O is performed here; the manager is populated by the `commands::document` adapter.
...
/// Opaque numeric identifier assigned to each open document.
...
pub struct DocumentState {
    #[allow(dead_code)] pub id: DocumentId,
    #[allow(dead_code)] pub path: String,
    pub buffer: Rope,
    pub syntax_tree: Option<Tree>,
    pub parser: Option<Parser>,
    pub highlight_query: Option<Query>,
    #[allow(dead_code)] pub language_id: String,
    pub version: usize,
}
...
impl DocumentState {
    ...
    #[allow(dead_code)] pub fn get_content(&self) -> String {
...
impl DocumentManager {
    ...
    #[allow(dead_code)] pub fn get_content(&self, id: DocumentId) -> Option<String> {
...
mod tests {
    ...
    #[test]
    fn document_state_new_creates_buffer() {
    ...
    fn apply_edit_increments_version() {
        ...
        doc.apply_edit(5, 11, 6, "!");
        assert_eq!(doc.version, 1);
        assert_eq!(doc.get_content(), "hello!");
    }

    #[test]
    fn byte_to_point_single_line() {
        let doc = DocumentState::new(
            2,
            "/test.rs".into(),
            "abc\ndef".into(),
        ...
        let pt = doc.byte_to_point(0);
        assert_eq!(pt.row, 0);
        assert_eq!(pt.column, 0);

        let pt2 = doc.byte_to_point(4);
        assert_eq!(pt2.row, 1);
        assert_eq!(pt2.column, 0);
    }
}
