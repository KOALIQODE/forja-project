//! Syntax highlighter — native parser binary → token stream.
//!
//! [`SyntaxHighlighter`] loads a compiled parser `.so`/`.dylib`/`.dll` from the
//! user's local cache, runs tree-sitter, and maps captures to [`TokenType`] values.
//! Falls back to a basic lexer when the parser isn't installed so the editor
//! can display files while the user installs grammars via Grammar Hub.
//!
...
use crate::domain::models::{SyntaxHighlight, Token, TokenType};
...
/// Stateless highlighter — all state is in the on-disk cache and lazy statics.
