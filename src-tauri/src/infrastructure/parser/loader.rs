...
mod tests {
    ...
    #[test]
    fn parser_loader_new_creates_empty_loader() {
        let loader = ParserLoader::new();
        assert!(loader.loaded_libraries.is_empty());
        assert!(loader.cached_languages.is_empty());
    }

    #[test]
    fn get_language_errors_for_nonexistent_binary() {
        let mut loader = ParserLoader::new();
        let result = loader.get_language("fake_parser", &PathBuf::from("/nonexistent/path/parser.so"));
