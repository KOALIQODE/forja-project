// diff/git_diff.rs — Phase 5
//
// GitDiffService: uses git2 to compute the diff between HEAD and the
// working-tree version of a file, returning structured GitHunk data.
//
// Also provides helpers for building unified-diff patch strings from a single
// hunk — used by stage_hunk and revert_hunk.
...
pub fn get_file_hunks(repo_path: &str, file_path: &str) -> Result<Vec<GitHunk>, String> {
    ...
    // Build diff: HEAD tree → working directory for this specific file
    ...
    for (delta_idx, _) in diff.deltas().enumerate() {
        ...
        for hunk_idx in 0..patch.num_hunks() {
            ...
            hunks.push(GitHunk {
                ...
                new_start: new_start.saturating_sub(1), // convert to 0-indexed
...
/// Used by the TS side to update its baseline when a file is first opened.
...
/// Used by stage_hunk / revert_hunk to construct an apply-able patch.
...
pub fn revert_single_hunk(repo_path: &str, file_path: &str, hunk: &GitHunk) -> Result<(), String> {
    ...
    // Reverse: swap +/- origins and swap old/new counts in header
...
fn build_reversed_patch(file_path: &str, hunk: &GitHunk) -> String {
    ...
    // Swap old/new in header:  "@@ -10,4 +12,6 @@"  →  "@@ -12,6 +10,4 @@"
...
fn reverse_hunk_header(_header: &str, hunk: &GitHunk) -> String {
    // Build a canonical reversed header rather than trying to parse the original
    format!(
        "@@ -{},{} +{},{} @@",
